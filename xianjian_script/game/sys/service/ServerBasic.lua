...
local ServerBasic=class("ServerBasic")


ServerBasic.overTimes = 0

--调试的errorCode 如果想指定服务器某一个协议(method)一些返回指定的errorCode, 那么就在这里配置
--示例 购买体力接口返回错误码 10017
-- ServerBasic.debugMethodErrorInfo = {
...
ServerBasic.connCache = nil 		--连接缓存 
ServerBasic.missCache = nil 		--连接未完成的 消息 数组 格式和conncache一样 但是 放入这个数组的 不会主动重发 只能被动 等待服务器回馈
...
ServerBasic.id =1
ServerBasic.curConn = nil 			--当前连接请求 一定是一个数组 至少是一维的 因为和服务器协议的就是允许多包合并
ServerBasic._isConnect = false 		--标记是否连接成功 	--

ServerBasic._isServerError= false  	--标记是否有服务器错误 如果有服务器错误 所有的请求不应该走过来

--模块的model映射表
ServerBasic.modelMap = nil 
--init消息发送
ServerBasic.OPCODE_KAKURA_INIT=1000
--心跳协议发送
ServerBasic.OPCODE_KAKURA_HEARTBEAT = 1001
--创建房间
ServerBasic.OPCODE_KAKURA_CREATEROOM = 1002
ServerBasic.OPCODE_KAKURA_JOINROOM = 1003
ServerBasic.OPCODE_KAKURA_EXITROOM = 1004
ServerBasic.OPCODE_KAKURA_REMOVEROOM = 1005
ServerBasic.OPCODE_KAKURA_GETROOMSUSERS = 1006
ServerBasic.OPCODE_KAKURA_COUNTUSERS = 1007
ServerBasic.OPCODE_KAKURA_KICKOUTUSERS = 1008
ServerBasic.OPCODE_KAKURA_KICKOUTROOMS = 1009
ServerBasic.OPCODE_KAKURA_PUSHMESSAGE = 1010
ServerBasic.OPCODE_KAKURA_TIMEEVENT = 1011
ServerBasic.OPCODE_KAKURA_REMOVEPUSHID = 1012
ServerBasic.OPCODE_KAKURA_CHECKONLINE = 1013
--//重新连接
ServerBasic.OPCODE_KAKURA_REAUTH = 1014
ServerBasic.OPCODE_KAKURA_NORESPONSEPUSHMESSAGE = 1016
        
ServerBasic.OPCODE_KAKURA_RESPONSE = 99998
ServerBasic.OPCODE_KAKURA_END = 99999

--//backend请求的opcode值，初始化之后的每个长连接请求都会发送这个code
ServerBasic.OPCODE_BACKEND_REQUEST = 100001

--//初始化的id值
ServerBasic.INIT_ID_VALUE = 1

-- 是否开启重连机制
ServerBasic.OPEN_REAUTH = true

--是否是切后台超时,因为现在切后台出去回来的时候 一般都会先serverClose
ServerBasic.isOverTimeByBackGround = false
--服务器类型     1是 gameserver  2 是battleserver  3是  其他的游戏服
ServerBasic._serverType = 1
...
--自动重连时间
ServerBasic._initReConnectTimes = 1
--自动重连间隔 
ServerBasic._initReConnectDelay = 2 
--剩余自动重连次数
ServerBasic._autoReconnectTime = 0


function ServerBasic:__init(  )
	self.id =1
	self.curConn = nil 			--当前连接请求 一定是一个数组 至少是一维的 因为和服务器协议的就是允许多包合并
	self._isConnect = false 		--标记是否连接成功 	--

	self._isServerError= false  	--标记是否有服务器错误 如果有服务器错误 所有的请求不应该走过来
	self.connCache = {}
	self.missCache = {}
	--剩余自动重连次数
...
-- isReconnect 是否是重连过来的
function ServerBasic:init(params,isReconnect)
	-- self.connCache = {}
	-- self.missCache = {}
	if self._isConnect then
		--如果是连接状态的 那么手动关闭
		echoWarn(self._isConnect,"为什么还是关闭状态-",self.__cname)
		self:handleClose(true)
	end
	--标记状态1 表示是激活状态
	self._state = 1
	self.isReconnect = isReconnect
	params = params or { ip = ServiceData.IP,port = ServiceData.PORT }
	...
	self.ignoreLogArr = {
		...
		MethodCode.battle_battleEnter  ,			--进入战斗
		MethodCode.explore_login_loginClient, 		-- 连接仙盟探索
	}
	--//关于发送的消息与opcode之间的映射表
    self.opcodeMap={
	--//初始化映射
      [MethodCode.sys_init]=ServerBasic.OPCODE_KAKURA_INIT,
      [MethodCode.sys_reauth] = ServerBasic.OPCODE_KAKURA_REAUTH,
	--//心跳映射
       [MethodCode.sys_heartBeat]=ServerBasic.OPCODE_KAKURA_HEARTBEAT,
    };
	...
	echo(params.ip, params.port,"params.ip, params.port")
	self.socket:init(params.ip, params.port, signSalt, 0,"0")
...
function ServerBasic:doInitRequest( )
	local clientInfo = AppInformation:getClientInfo(true)
	...
	if not self:checkHasMethod(self:getInitReqeustId()) then
		...
		local   _init_param={
             ...
             token = LoginControler:getToken(  ),
             sec = LoginControler:getServerId(),
             queueTime = LoginControler:getQueueTime()
		};
...
--判断是否拥有哪个请求
function ServerBasic:checkHasMethod( method )
	if self.curConn then
...
--移除某个请求
function ServerBasic:clearByMethodId( method )
	if self.curConn then
		for i,v in ipairs(self.curConn) do
			if v.method == method then
				self.curConn = nil
...
--连接完成后的回调
function ServerBasic:onConnectBack( result )
	--如果已经是关停状态 那么不需要
	...
	if  (not result) or  (not result.result) then
		--那么一定要弹到主界面
		self:checkError( result, true,true)
		-- 2018.06.20 checkError移到该代码之前，因为在选服失败时需要判断是否是因为排队导致的
		-- checkError 中会获取到失败原因(如果是排队导致，会保存相关数据)
		if self:isGameServer() then
			-- 重连不发初始化失败
			if not self:isGetUserInfo() then
				EventControler:dispatchEvent(LoginEvent.SERVEREVENT_INIT_FAIL)
			end
		end

		return
	end
	dump(result,"onConnectBackInfo")
	--初始化自动重连次数
	self._autoReconnectTime = self._initReConnectTimes
...
--//注意,目前已经没有了result.aesKey,所以需要传空字符串,而非result.result.aesKey
	...
	--echo("ServerBasic:onConnectBack 连接返回---是否获取过用户信息:", self:isGetUserInfo() ,"initId:",result.result.initRequestId)
	...
	if self:isGameServer() then
		...
		if not self:isGetUserInfo( ) then
			--请求用户信息
			getUserInfo = self:turnRequestSave( {},  MethodCode.user_getUserInfo_301  )

			table.insert(self.connCache, 1,{getUserInfo} )

			-- LoginControler:doGetUserInfo()
		else --//否则是掉线后重新登录
	        -- LoginControler:doGetUserDataAfterOffline();
	        getUserInfo = self:turnRequestSave( {},  MethodCode.user_relogin_359 ,c_func(LoginControler.doReloginInfoBack, LoginControler ) )
		...
		--需要把这个请求插到最前面去
		self:doRequest()
	elseif self:isRealTimeServer() then
		self:doRequest()
		--发送一个战斗重连上了的消息 通知客户端自己去做处理
		FightEvent:dispatchEvent(BattleEvent.BATTLEEVENT_ONSERVER_CONNECT)
	else
		--服务器请求成功
		self:afterServerOnConnect()
	end
...
function ServerBasic:checkRequestId(  )
...
function ServerBasic:onCallback(event, data, uniqueId) 
	...
	if event ==ServiceData.MESSAGE_ERROR  then
		if data and data ~= "error" then
			...
			if not table.isEmpty( jsonData) then
				...
				echo(self.__cname, "onerrorMessage:",data)
				self:onError()
			end
...
--modify ,目前由于网络框架的改变,已经不能使用这种方法来判断是否是初始化成功的 标志,需要使用新的
        ...
        if(jsonData.result and jsonData.result.initRequestId~=nil )then
             self.curConn=nil;
             self:onConnectBack(jsonData);
             return;
        end
...
--		end
	...
	if DEBUG_CONNLOGS > 0  then
		--请求GM的接口不存储
		if  not (jsonData and jsonData[1] and (jsonData[1].method == 100104 or jsonData[1].method == 100106) ) then
			self:saveLogs(data,"callBack" .. "_" .. tostring(TimeControler:getServerTime()));
		end
		
		if DEBUG_CONNLOGS == 2 then
			if self.ignoreLogArr then
				local backMethod = jsonData[1].method
				if tonumber(backMethod) then
					backMethod = toint(backMethod) -1
				end
				if table.indexof(self.ignoreLogArr, backMethod )  then
					--不是心跳请求
					if jsonData[1].method ~= 320 then
						echo(self.__cname.. "_callBack:".."忽略掉输出的信息,method:"..jsonData[1].method)
					end
					...
					if jsonData[1].result ~= "success" then
						...
						--最多截取1000字节
						print(  "[echo:".. (os.date("%d-%H:%M:%S",os.clock() )) .."]"  , "rid:"..UserModel:rid().."_".. self.__cname.. "_callBack" .. "_time" .. tostring(TimeControler:getServerTime()) .. ":" .. string.sub(data, 1,targetIndex))
					end
	...
	for i,v in ipairs(data) do
		...
		-- 	echo(json.encode(v).."__error_info")
			...
			if NotifyEvent[tostring(v.method)] then-- and v.method ~= MethodCode.user_state_315+1  then--//同时将init消息排除掉
				if v.params and v.params.data and v.params.data.dirtyList then
					self:updateBaseData(v.params.data.dirtyList)
				end
				...
				if self:isGameServer() then
					...
					--如果是获取用户信息返回
					if tonumber(v.method) == MethodCode.user_getUserInfo_301 +1  and  (not self:isGetUserInfo())  then
						LoginControler:doGetUserInfoBack(v)
					end
				end
	...
	if dataTime.battleServerTime then
		TimeControler:updateBattleServerTime(dataTime.battleServerTime)
	end

	local time2=os.clock()
	local costTime = time2 - time1
	local method = jsonData[1].method
	if costTime > GameVars.costTimeCfg[device.platform] then
		echoWarn ("协议耗时=",costTime,",method=",method)
	end
end

--执行通知
function ServerBasic:onNotify( notifyArr )
...
--执行回调
function ServerBasic:onRequestBack( respone )
	-- if not respone then
	-- 	return
	-- end
	respone = respone or {}
	...
	--必须不是心跳请求 才关闭loading  这也是为什么没有阻拦一些请求的原因
	if  respone[1].result ~= "success"then
		-- WindowControler:hideLoading()
		self:showOrHideLoading(false)
	end
	...
	if self.curConn then
		--判断当前回调
		--这里防止回调有卡死 采用pcall
		local tempFunc = c_func(self.excuteOneResponceGroup,self,respone, self.curConn, 1)
...
--解析一个消息组
function ServerBasic:excuteOneResponceGroup(responceArr, connInfo ,groupType)
	if #connInfo ==0 then
		return
	end
...
--解析一条返回信息
function ServerBasic:excuetOneResponce(info, responceArr )
	...
	for i=length,1,-1 do
		...
		if self:isGameServer() then
			methodid = tonumber(resp.method) 
			if methodid and  table.find(NoErrorTipsCode,methodid-1) then
				hideCommonTips = true
			end
		else
			methodid = resp.method 
		...
		if not methodid then
			...
			if resp.result and resp.result == "success" then
				...
				echoWarn("返回的method不是数字,", resp.method,self:isGameServer())
				-- dump(resp,"__resp")
			end
		...
		if self:isGameServer() then
			...
			if tostring(methodid) ==  tostring(info.method) then
				isMatched = true
			end
		end
		

		if self:isRealTimeServer() then
			echo(methodid,info.method,info.uniqueId, resp.uniqueId,":dsadsadsad",isMatched)
		end

		if (  (not info.uniqueId   ) or (not resp.uniqueId)    or (info.uniqueId and resp.uniqueId == info.uniqueId ) ) 
				and isMatched  then
		-- if  methodid == tonumber(info.method)  then
			--判断这个消息是否已经处理过
			local canDo =true
...
function ServerBasic:checkMethod(  )
	-- body
end
...
function ServerBasic:updateBaseData(baseData,isServerBack)
	-- echo("--------------------------------------")
	-- dump(baseData)
	-- echo("--------------------------------------")
	--先把 非userdata的数据拆开
	...
	--需要优先更新的模块-- 比如 战力一定要在伙伴系统之前算. 
	local priorArr = {"abilityNew"}
	...
	if changeData then
		userHasChange =false
		for i,v in ipairs(priorArr) do
			if changeData[v] and self.modelMap[v] then
				self.modelMap[v]:updateData(changeData[v])
				changeData[v] = nil
			end
		end
...
--判断异常
function ServerBasic:checkError( responseInfo, hideCommonTips,isInitError)
	...
	--发送一个错误码
	EventControler:dispatchEvent(SystemEvent.SYSTEMEVENT_ONSERVERERROR  ,errorInfo.code )
    ...
    elseif errorCode == ErrorCode.kakura_server_error then
        ...
        return
    -- 账号被封禁
    elseif errorCode == ErrorCode.account_forbided
    	then
    	self.curConn = nil
		self._isOffline=true;
        local ui =WindowControler:showHighWindow("CompServerOverTimeTipView")
    ...
    elseif errorCode == ErrorCode.logintoken_expire
        ...
        ui:setCallFunc(c_func(WindowControler.goBackToEnterGameView,WindowControler),true,errorInfo)
       	self._isServerError = true
        return
    elseif errorCode == ErrorCode.user_need_queue_time then
    	-- 将自动重连设置为0
    	self._autoReconnectTime = 0
    	local data = errorInfo.data
    	dump(data,"data---------------")
    	LoginControler:setQueueData(data)
    	WindowControler:showHighWindow("LoginQueueUpView")
    	return
    elseif errorCode == ErrorCode.battle_lose then
    	WindowControler:showTips(GameConfig.getLanguage("#tid_team_tips_006"))
	...
	if  not tip then
		tip = "local network error"
	end
	-- local sendMsg = tip .."\n errorCode:"..errorCode .."_method:".. tostring(responseInfo.method)
	-- ClientActionControler:sendErrorLogToWebCenter(ClientActionControler.LOG_ERROR_TYPE.TYPE_LUA,ClientTagData.socketServerError.."_"..self.__cname,sendMsg)
...
--(错误回调和正确回调都是调用同一个函数)
-- isInsert 是否需要插队请求 1是插队 默认nil 或者其他值 表示不插队
function ServerBasic:sendRequest(params, methodid, callBack, oneway, outLoading, needErrorCall,isInsert)
	--游戏被销毁的时候 就不处理了
    if GameLuaLoader:isGameDestory() then
    	echoError("__GameLuaLoader:isGameDestory___is")
        return
    end
	--如果是心跳 而且,没连上 那么不重连
	...
	-- echo(methodid,oneway,"__fsdjahdjshfjsdhfjsdhfjshjh")
	--如果是请求协议被临时关闭
	...
	local tempFunc = function ()
		...
		if self.curConn and #self.curConn > 0 then
			...
			if self._isClose and not self._isOffline and  methodid ~= self:getInitReqeustId()  then
				...
				self:init(self.serverParams,true)
				return
			end
...
--判断当前是否正在请求
function ServerBasic:checkIsSending(  )
	if self.curConn then
		return true
	end
	return false
...
function ServerBasic:sendGroupRequest( paramsGroup )
	-- body
...
--转化成 保存的请求格式
--此时生成uniqueId
function ServerBasic:turnRequestSave(params, methodid, callBack, oneway, outLoading, needErrorCall)
	--self.id = self.id + 1 
	local info = {}
	-- info.id = self.id
	info.params = params
...
--转化保存的格式 为 请求发送格式
function ServerBasic:turnSendRequest( connInfo,onlyStr )
	local sourceArr 
	--如果是单个请求 那么需要包装一下
	...
	for i,v in ipairs(sourceArr) do
		...
		if tempObj.method == self:getInitReqeustId() or tempObj.method == MethodCode.sys_heartBeat then
			tempObj.id = ServerBasic.INIT_ID_VALUE	
		else
	...
	if DEBUG_CONNLOGS >0 then
		self:saveLogs(str,self.__cname.. "_connectinfo" .. "_" .. tostring(TimeControler:getServerTime()))
		if DEBUG_CONNLOGS ==2 then
			result.token = nil
			--输出没有token的信息
			...
			if sourceArr[1].method ~= MethodCode.sys_heartBeat and  sourceArr[1].method ~= MethodCode.user_getOnlinePlayer_319  then
				echo("rid:"..UserModel:rid().."_".. self.__cname.. "_connectinfo" .. "_time" .. tostring(TimeControler:getServerTime()) ..  ":" .. string.sub(logStr, 1,1000))
			end
		end
	end
...
-- 重新发送当前请求
function ServerBasic:reSendRequest()
	--有系统错误的时候 return掉
	if self._isServerError then
		return
	end
...
-- 取消当前请求
function ServerBasic:cancelRequest()
	echo("cancelRequest self.curConn=");
end

-- 发送请求到服务器
function ServerBasic:doRequest()
	if self.curConn then
		return
	end
...
--确定发送
function ServerBasic:sureSend( info )
	local needLoading =false
	local isOneWay = false
	...
	--需要经过一道数据转化
	--//获取opcode 
	local   _opceode=self.opcodeMap[info[1].method]   or ServerBasic.OPCODE_BACKEND_REQUEST;
	local   _send_format ,_id, uniqueId = self:turnSendRequest(info);
	local tempFunc = function (  )
		self.socket:sendRequest(_opceode, _id, _send_format, uniqueId);
	end
	if IS_NETWORK_DELAY and IS_NETWORK_DELAY > 0 then
		WindowControler:globalDelayCall(tempFunc, IS_NETWORK_DELAY/1000)
	else
		tempFunc()
	end
...
function ServerBasic:checkOverTime( curConn )
	--关闭loading
	-- WindowControler:hideLoading()
	self:showOrHideLoading(false)
    ...
    self.overTimes = self.overTimes +1
	self._delayCallId = nil
	-- if self.curConn ~= nil then
	-- 	--那么设置重连函数
	...
	if curConn then
		...
		if self:isGameServer() then
			...
			--战斗服超时2次还没练上
			if self.overTimes >2 then
				local errorInfo = {error ={code = ErrorCode.sys_overTime_error,message = "server is error" } }
...
-- 网络连接已关闭
function ServerBasic:onClose(jsonData)
	--如果还没连接成功的时候 那么是不需要close的
	-- echo("ServerBasic:onClose",self._isConnect);
	-- if not self._isConnect then
	-- 	return
	-- end
	...
	echo(self.__cname.. " onClose",self._isConnect,self.curConn);
	self.socket = nil
	self._isClose = true
	--取消连接
	self._isConnect =false
	...
	echo(self._autoReconnectTime,"-self._autoReconnectTime")
	--自动重连y

	if not self._isOffline then

		if self._autoReconnectTime > 0    then
			self._autoReconnectTime = self._autoReconnectTime -1
			WindowControler:globalDelayCall(c_func(self.reSendRequest,self), self._initReConnectDelay)

		else

			--如果是战斗服 重连2次失败 走新的逻辑
			if not self:isGameServer() then
				--做连接是失败处理,交给ServerRealTime
...
function ServerBasic:onReconnectFail( ... )
end

-- 没有网络
function ServerBasic:onError(jsonData)
	echo("ServerBasic:onError");
	if not self._isConnect then
		return
	end
	...
	self:saveLogs("ServerBasic on close", "warn")
...
function ServerBasic:doAllRequestErrorCall( errorInfo )
	...
	local  missCache = self.missCache
	self.missCache = {}
	for i,v in ipairs(missCache) do
		doCallFunc(v)
		echoWarn("__做missle消息的错误回调")
	end

	local connCacheArr = self.connCache
	self.connCache = {}
	for i,v in ipairs(connCacheArr) do
		doCallFunc(v)
		echoWarn("__做缓存消息的错误回调")
	end
...
--输出一个消息组信息
function ServerBasic:toConnString( conninfo )
	local str = ""
	for i,v in ipairs(conninfo) do
...
--输出一个responcestring
function ServerBasic:toResponceString( responce )
	local str =""
...
--手动关闭
function ServerBasic:handleClose(isInit)
	if not isInit then
		--初始化状态为 为连接
		self._state = 0
	end
	...
	self:showOrHideLoading(false)
	--移除注册的事件.防止退出某个系统之后又重复注册事件 导致错误
	EventControler:clearOneObjEvent(self)
end

function ServerBasic:isConnect()
	return self._isConnect
end

function ServerBasic:deleteMe()
	if self.socket then
		self.socket:release()
		--self.socket:release()
		self.socket = nil
...
--保存日志
function ServerBasic:saveLogs( str,title )
	--如果是调试日志的
...
--获取初始化请求协议和参数
function ServerBasic:getInitReqeustId(  )
	if self:isGetUserInfo() then
		return  MethodCode.sys_reauth
	end
	return MethodCode.sys_init 
end
...
--是否已经获取过用户信息
function ServerBasic:isGetUserInfo(  )
	return self._hasGetUserInfo
end

--设置是否获取了用户信息
function ServerBasic:setHasGetUserInfo(value )
	self._hasGetUserInfo = value
end

function ServerBasic:showOrHideLoading( value )
	if  self:isRealTimeServer() then
		return
	end
	if value then
...
function ServerBasic:resetServerError(  )
	self._isServerError =false
end

function ServerBasic:isGameServer(  )
	return self._serverType == ServiceData.serverTypeMap.gameServer
end
function ServerBasic:isRealTimeServer(  )
	return self._serverType == ServiceData.serverTypeMap.realTimeServer
end
