local MemoryPingLunView = class("MemoryPingLunView", UIBase);

function MemoryPingLunView:ctor(winName)
    MemoryPingLunView.super.ctor(self, winName)
end

function MemoryPingLunView:loadUIComplete()
    self:registerEvent()
end 

function MemoryPingLunView:registerEvent()
    MemoryPingLunView.super.registerEvent(self);
...
function MemoryPingLunView:clickClose()
...
function MemoryPingLunView:setBarrageUI()
    local frame = 1
...
function MemoryPingLunView:setBarrageButton()
...
-- 仅有评论
function MemoryPingLunView:getServerData(arrayData)
...
function MemoryPingLunView:initData(arrayData,commentsInfo)
...
--设置输入框
function MemoryPingLunView:setInputButton()
    
    self.panel_2.btn_1:setTouchedFunc(c_func(self.sendServerData, self))
end

function MemoryPingLunView:sendServerData()
        ...
        if count - suncount >= 0 then
            local str = GameConfig.getLanguage("#tid_memory_017")
            if  self.arrayData.systemName == FuncCommon.SYSTEM_NAME.PARTNER then
...
function MemoryPingLunView:sendContentToServer(_text)
    -- echo("=====发送评论到服务器===",_text)
...
--设置滚动条数据
function MemoryPingLunView:setScrollData(data)
...
function MemoryPingLunView:setCellView(baseCell, itemData)
    ...
    --是不是热评
    -- echo("hot = = = == = = = ",itemData.hot)
    -- echo("tmpNum11111 = = = = = ",self.tmpNum)
    local isbuzz = false 
    ...
    local isXJZJ = false
    if itemData.id and itemData.id == self.tmpID then
        isXJZJ = true
    end
    baseCell.panel_zj:setVisible(isXJZJ)
    ...
    --赞
    panel.mc_1:setTouchedFunc(c_func(self.buttonGood, self,itemData,panel,baseCell));
    --踩
...
function MemoryPingLunView:showReportButton(baseCell,itemData)
    -- echo("======举报======")
    baseCell.btn_jb:setVisible(true)
    baseCell.btn_jb:setTouchedFunc(c_func(self.SendReportButton, self,itemData));
end
function MemoryPingLunView:SendReportButton(itemData)
    -- dump(itemData," \n\n显示玩家评论的数据 ====== ")
...
function MemoryPingLunView:buttonGood(itemData,panel,baseCell)
    echo("=======赞=======")
...
function MemoryPingLunView:buttonPoor(itemData,panel)
    local function _callback(param)
        if param.result ~= nil then
...
function MemoryPingLunView:close()

    self:startHide()
end

function MemoryPingLunView:deleteMe()
    -- TODO
    MemoryPingLunView.super.deleteMe(self);
end

return MemoryPingLunView;
