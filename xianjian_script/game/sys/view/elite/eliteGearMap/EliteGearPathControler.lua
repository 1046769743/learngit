--
--Author:      zhuguangyuan
--DateTime:    2018-02-08 11:31:29
--Description: 机关光路寻路控制器
--
...
-- startPoint = {x=1 ,y=1}
-- 返回值：是否找到 路径数组
function ElitePathControler:findPath(startPoint,endPoint)
	self:resetData()
...
-- 获取格式化路径
function ElitePathControler:getformatPath(list,endPoint) 
	local pointPath = {}
	-- 如果找到了目标点
	if endPoint then
...
function ElitePathControler:getPathLength(path) 
...
-- 找出targetPoint的相邻节点
function ElitePathControler:getSurroundPoints(targetPoint)
...
-- 检查阻挡
function ElitePathControler:checkBlock(x,y)
