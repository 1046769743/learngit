...
local EliteBasicModel = class("EliteBasicModel")


-- 指针数据
EliteBasicModel.controler =nil 		-- 游戏控制器
EliteBasicModel.myView = nil 		-- 视图  ViewBasic 对象
EliteBasicModel.shade = nil 			-- 影子 ModelShade对象
EliteBasicModel.effectArr = nil		-- 特效的数组
--[[
shakeInfo = {
		frame = frame, 			--震动帧数
...
EliteBasicModel.shakeInfo = nil 		--自身震屏信息

EliteBasicModel.depthType = 0 		-- 深度排列的类型 	 同一y下的时候 根据这个决定深度 类型越高越在里面
EliteBasicModel.modelType = 0 		-- model类型 


-- 游戏速度
EliteBasicModel.updateScale =1 		-- 刷新比率  如果scale>1 表示快动作  小于1 表示慢动作
EliteBasicModel.updateCount = 0 		-- 刷新计数 
EliteBasicModel.updateScaleCount = 0 -- 游戏速度
EliteBasicModel.lastScaleTime = -1   -- 加速时间计时

--各种暂停
EliteBasicModel.skillPause = false 	-- 技能导致暂停  
EliteBasicModel.selfPause = false 	-- 代码暂停
...
EliteBasicModel.viewScale =  1 		-- 试图的scale

--坐标和层级
EliteBasicModel.__zorder = 0 		-- zorder
EliteBasicModel.pos = nil 			--坐标 {x,y,z}

EliteBasicModel._isDied = false

--战队信息
EliteBasicModel.initCamp = nil  		--初始阵营,需要记录这个值 如果我方某个人被魅惑了 那么这个人是不能被攻击的 

EliteBasicModel.camp = 1 			--阵营
EliteBasicModel.way = 1 				--x的运动方向 初始化默认为1 就是朝右的

EliteBasicModel.campArr=nil 			--我的阵营队伍
EliteBasicModel.toArr = nil 			--敌人阵营数组  如果以后扩展多方阵营 那么会 扩展 更多toArr   和campArr 
EliteBasicModel.callFuncArr = nil 	

EliteBasicModel._viewScale = 1 		--视图缩放系数 

function EliteBasicModel:ctor( controler )
	--self.countId = 0
...
function EliteBasicModel:registerEvent()
	
end

function EliteBasicModel:initView(ctn,view,xpos,ypos,zpos,size)
	
	--容器层
	self.viewCtn = ctn
	self.myView = view
	ctn:addChild(self.myView)
	-- self.myView:anchor(0,0)
...
-- 当view创建完成
function EliteBasicModel:onInitViewComplete()

end

--设置坐标
function EliteBasicModel:setPos(xpos ,ypos ,zpos  )
...
--设置刷新速度  比如快动作
function EliteBasicModel:setUpdateScale(scale,lastTime)
...
--更新视图速度
function EliteBasicModel:updateViewPlaySpeed( )
...
--设置方位
function EliteBasicModel:setWay( way )
	if not way then
		return
	end
...
--设置viewscale
function EliteBasicModel:setViewScale( value )
...
function EliteBasicModel:getViewScale( )
	return self.viewScale or 1
end

function EliteBasicModel:setViewSize( viewSize )
...
--停止播放动作
function EliteBasicModel:stopFrame(  )
	self.selfPause = true
	self:checkCanPlayView()
end

--恢复播放动作
function EliteBasicModel:playFrame(  )
	self.selfPause = false
	self:checkCanPlayView()
end

--游戏暂停或者播放
function EliteBasicModel:gamePlayOrPause( value )
	self:checkCanPlayView()
end

--场景暂停或者播放
function EliteBasicModel:scenePlayOrPause( value )
...
function EliteBasicModel:shake( frame,range,shakeType  )
...
--刷新函数
function EliteBasicModel:updateFrame( )
...
--按照加速比率进行刷新
function EliteBasicModel:runBySpeedUpdate( ... )
...
--硬直事件
function EliteBasicModel:myStillMoment( ... )
...
--硬直事件
function EliteBasicModel:still()
...
--跳出硬直
function EliteBasicModel:outStill(  )
...
function EliteBasicModel:setStill(time,type,x,y,r )
...
function EliteBasicModel:isStill(  )
	return  false
end

--抖动 	持续帧  力度    震屏方式 1,x 2,y 3 xy 方向震动
function EliteBasicModel:selfShake( frame,range,shakeType )
...
--帧事件
function EliteBasicModel:dummyFrame( ... )
end

--一些控制事件 --供子类重写
function EliteBasicModel:controlEvent(  )
end

--更新速度
function EliteBasicModel:updateSpeed( ... )
end

--碰撞检测
function EliteBasicModel:checkHit( ... )
end

--移动坐标
function EliteBasicModel:moveXYZPos( ... )
end
...
--转换真实坐标
function EliteBasicModel:realPos( )
...
function EliteBasicModel:deleteMe( ... )
	self._isDied = true
...
function EliteBasicModel:pushOneCallFunc( delayFrame,func,params )
...
function EliteBasicModel:updateCallFunc(  )
...
----------------------------------------------------------------------------------
--重新计算view的scale
function EliteBasicModel:countScale( )
...
--闪光
function EliteBasicModel:flash(time,interval, color  )
...
--判断是否有滤镜样式 供子类重写
function EliteBasicModel:checkHasFilterStyle(  )
	return false
end


--创建残影 组
function EliteBasicModel:createGhostGroup(times,interval, offset, zorder,ctn ,alpha, lastTime)
...
--创建残影
function EliteBasicModel:createGhost( x, y, zorder,ctn ,alpha, lastTime)
	alpha = alpha or 0.3
...
--显示或者隐藏view
function EliteBasicModel:setVisible( value )
...
function EliteBasicModel:setZOrder( zorder )
	self.zorder = zorder
	if self.myView then
...
function EliteBasicModel:getZOrder( )
	return self.zorder
end

function EliteBasicModel:getSortPos()
	return self.pos
end

function EliteBasicModel:getContentSize()
	return self.mySize
end

function EliteBasicModel:tostring(  )
	return "className:"..self.__cname .."_pos:"..self.pos.x.."_"..self.pos.y.."_"..self.pos.z
end

return EliteBasicModel