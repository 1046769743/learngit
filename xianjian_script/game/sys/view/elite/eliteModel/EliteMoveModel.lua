...
local EliteMoveModel = class("EliteMoveModel",EliteBasicModel)


--[[-速度也变成{x=0,y=0,z=0}的格式 为了统一 ]]
EliteMoveModel.speed = nil
EliteMoveModel.addSpeed = nil 			--加速度
EliteMoveModel.myState = Fight.state_stand 	--状态 


EliteMoveModel.gravitiAble = true 	--能否受重力
 
 --运动类型
EliteMoveModel.moveType =  0 	--运动类型
EliteMoveModel.movePostion = nil 	 --运动到点的坐标
EliteMoveModel.movePointsInfo = nil 	-- 根据一系列点来运动

EliteMoveModel.viewMoving = false -- 视图移动控制

function EliteMoveModel:ctor( ... )
 	EliteMoveModel.super.ctor(self,...)
 	self.movePointsInfo = {}
...
function EliteMoveModel:initView(...)
	EliteMoveModel.super.initView(self,...)
end

--初始化状态
function EliteMoveModel:initStand( )
...
function EliteMoveModel:initJump(jumpSpd )
	self.moveType=0
...
function EliteMoveModel:initMove(xspd,yspd,minXSpd,minYSpd)
	if xspd then
		self.speed.x = xspd
	end
	if yspd then
...
--执行运动函数
function EliteMoveModel:move( spd )
...
--执行跳跃操作
function EliteMoveModel:jump( spd )
	if spd ~= 0 then
		self:initJump(spd)
	else
...
-- 设置加速度
function EliteMoveModel:setAddSpeed(xspd ,yspd ,zspd  )
	if xspd then
...
--设置重力加速度
function EliteMoveModel:setGravity( gravity )
	self.addSpeed.z = gravity
end

--设置速度
function EliteMoveModel:setSpeed(xspd ,yspd ,zspd  )
...
--重写控制事件
function EliteMoveModel:controlEvent(  )
	EliteMoveModel.super.controlEvent(self)

	self:checkMoveType()	
end

-- 判断运动类型
function EliteMoveModel:checkMoveType(...)
...
--更新速度
function EliteMoveModel:updateSpeed( ... )
...
--移动坐标
function EliteMoveModel:moveXYZPos()
...
--运动到目标了
function EliteMoveModel:overTargetPoint(  )
	self:initMoveType()
	self:initStand()
end
...
function EliteMoveModel:moveToPoint(targetPoint, speed,moveType )
...
function EliteMoveModel:calRadian(targetPoint)
...
function EliteMoveModel:calAngle(targetPoint)
	local radian = self:calRadian(targetPoint)
	return radian * 180 / math.pi
end

function EliteMoveModel:onMoveToPointCallBack()
	echo("\nEliteMoveModel:onMoveToPointCallBack")
end

--运动函数  根据一系列点运动						重复类型 0 表示不重复 1表示重头开始 2表示随机点序列以后重复
function EliteMoveModel:moveByPointArr( pointArr,speed,repeateType )
	-- body
...
--映射到目标点 pos(x,y,z)
function EliteMoveModel:mapSpeedToTargetPos( pos,spd,viewRota )
...
--初始化运动类型
function EliteMoveModel:initMoveType( ... )
	self.moveType =0
	self.movePostion=nil
end

--计算速度
function EliteMoveModel:countSpeed( targetX,targetY,frame ,minSpeed )
	frame  =  frame or 10
...
function EliteMoveModel:getWorldPos()
	local point = {x=0,y=0}
	if self.myView then
...
-- 是否在屏幕可见范围内
function EliteMoveModel:isInScreen(  )
