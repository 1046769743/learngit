...
function EliteMap:initData()
	...
	self.rightDoorWidth = GameVars.gameResWidth/7
	self.bottomDoorWidth = GameVars.gameResHeight*9/32
	local mapTailOffsetX = 170
	self.minMapX = 0
	self.maxMapX = (self.xNum / 2) * gridWidth - (GameVars.width - self.rightDoorWidth) + mapTailOffsetX

	-- 地图格子皮肤
	self.mapGridSkin = EliteMapModel:getEliteMapGridSkin(EliteMainModel:getCurrentChapter())
end
...
function EliteMap:initLayers()
	...
	local bg = display.newSprite(FuncRes.iconBg( "elite_bg_changjing" ))
	bg:parent(self.backLayer,10):pos(GameVars.gameResWidth/2,-GameVars.gameResHeight/2+GameVars.UIOffsetY) --(GameVars.height-GameVars.gameResHeight)
	...
	local middleLayerY = -(GameVars.gameResHeight - self.mapControler.gridHeight * self.yNum)
	local offsetX = self.mapControler.gridWidth + self.rightDoorWidth -- GameVars.UIOffsetX*0.03
	local offsetY = self.mapControler.gridHeight*2 + self.bottomDoorWidth -- GameVars.UIOffsetY*0.07
	self.gameMiddleLayer:pos(GameVars.gameResWidth - offsetX,-GameVars.gameResHeight + offsetY)
