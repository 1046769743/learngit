...
local WorldBasicModel = require("game.sys.view.world.model.WorldBasicModel")

local WorldMoveModel = class("WorldMoveModel",WorldBasicModel)


--[[-速度也变成{x=0,y=0,z=0}的格式 为了统一 ]]
WorldMoveModel.speed = nil
WorldMoveModel.addSpeed = nil 			--加速度
WorldMoveModel.myState = Fight.state_stand 	--状态 


WorldMoveModel.gravitiAble = true 	--能否受重力
 
 --运动类型
WorldMoveModel.moveType =  0 	--运动类型
WorldMoveModel.movePostion = nil 	 --运动到点的坐标
WorldMoveModel.movePointsInfo = nil 	-- 根据一系列点来运动

WorldMoveModel.viewMoving = false -- 视图移动控制

function WorldMoveModel:ctor( ... )
 	WorldMoveModel.super.ctor(self,...)
 	self.movePointsInfo = {}
...
function WorldMoveModel:initView(...)
	WorldMoveModel.super.initView(self,...)
end

--初始化状态
function WorldMoveModel:initStand( )
...
function WorldMoveModel:initJump(jumpSpd )
	self.moveType=0
...
function WorldMoveModel:initMove(xspd,yspd,minXSpd,minYSpd)
	if xspd then
		self.speed.x = xspd
	end
	if yspd then
...
--执行运动函数
function WorldMoveModel:move( spd )
...
--执行跳跃操作
function WorldMoveModel:jump( spd )
	if spd ~= 0 then
		self:initJump(spd)
	else
...
-- 设置加速度
function WorldMoveModel:setAddSpeed(xspd ,yspd ,zspd  )
	if xspd then
...
--设置重力加速度
function WorldMoveModel:setGravity( gravity )
	self.addSpeed.z = gravity
end

--设置速度
function WorldMoveModel:setSpeed(xspd ,yspd ,zspd  )
...
--重写控制事件
function WorldMoveModel:controlEvent(  )
	WorldMoveModel.super.controlEvent(self)

	self:checkMoveType()	
end

-- 判断运动类型
function WorldMoveModel:checkMoveType(...)
...
--更新速度
function WorldMoveModel:updateSpeed( ... )
...
--移动坐标
function WorldMoveModel:moveXYZPos()
...
--运动到目标了
function WorldMoveModel:overTargetPoint(  )
	self:initMoveType()
	self:initStand()
end
...
function WorldMoveModel:moveToPoint(targetPoint, speed,moveType )
...
function WorldMoveModel:onMoveToPointCallBack(isEnd)
	echo("WorldMoveModel:onMoveToPointCallBack")
end

--运动函数  根据一系列点运动						重复类型 0 表示不重复 1表示重头开始 2表示随机点序列以后重复
function WorldMoveModel:moveByPointArr( pointArr,speed,repeateType )
	-- body
...
--映射到目标点 pos(x,y,z)
function WorldMoveModel:mapSpeedToTargetPos( pos,spd,viewRota )
...
--初始化运动类型
function WorldMoveModel:initMoveType( ... )
	self.moveType =0
	self.movePostion=nil
end

--计算速度
function WorldMoveModel:countSpeed( targetX,targetY,frame ,minSpeed )
	frame  =  frame or 10
...
-- 暂停model动画及逻辑
function WorldMoveModel:pauseMe()
...
-- 恢复model动画及逻辑
function WorldMoveModel:resumeMe()
