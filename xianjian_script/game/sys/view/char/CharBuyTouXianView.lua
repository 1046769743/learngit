...
function CharBuyTouXianView:TuiChuJView( )
	WindowControler:showTips(GameConfig.getLanguage("#tid_title_002"))
end
function CharBuyTouXianView:ShowData(TouXianId)
	...
	self.UI_1.txt_1:setString(GameConfig.getLanguage("#tid_title_003"))
	self.UI_1.mc_1:getViewByFrame(1).btn_1:getUpPanel().txt_1:setString(GameConfig.getLanguage("#tid_title_004"))
end
		...
		-- if tonumber(coin)  >= tonumber(costs[2]) then
			local callBack = function ( params )
	            ...
	            if params.result ~= nil then
	            	self:initData()
	            	WindowControler:showTips(GameConfig.getLanguage("tid_common_2024"))
