...
import ItemChar from '../View/Game/ItemChar';
...
export class EffectManager extends cc.Component {
    ...
    @property({ type: cc.Prefab })
    public AddCashOut: cc.Prefab = null;
    ...
    public playCollectEffect(collectType: CollectEffectType, num: number, startPos: cc.Vec2, targetPos: cc.Vec2, callback: Function) {
        ...
        } else if (collectType == CollectEffectType.CashOut) {
            effectPrefab = this.AddCashOut;
        } else {
    ...
    // 播放收集字母动画
    public playCollectCharEffect(prefab: cc.Prefab, cellSize: number, char: string, startPos: cc.Vec2, targetPos: cc.Vec2, callback: Function) {
        let localStartPos = this.node.convertToNodeSpaceAR(startPos);
        let localTargetPos = this.node.convertToNodeSpaceAR(targetPos);
        let item = ObjectPoolManager.instance.getNode(prefab);
        item.parent = this.node;
        item.position = localStartPos;

        let itemChar = item.getComponent(ItemChar) as ItemChar;
        itemChar.updateChar(char, cellSize);
        let fontSize = Math.max(cellSize / 2, 40);
        let targetScale = 40 / fontSize;

        let tw = new cc.Tween(item);
        tw.parallel(
            cc.tween().to(0.5, { position: localTargetPos }, { easing: 'sineInOut' }),
            cc.tween().to(0.5, { scale: targetScale }, { easing: 'sineInOut' })
        );
        tw.call(() => {
            itemChar.reset();
            ObjectPoolManager.instance.putNode(item);
