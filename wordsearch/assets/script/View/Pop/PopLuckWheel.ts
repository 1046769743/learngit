...
export default class PopLuckWheel extends BasePop {
    ...
    // 普通按钮
    @property(cc.Node)
    nodeNormalBtnGray: cc.Node = null;
    @property(cc.Node)
    nodeNormalBtnSpin: cc.Node = null;
    @property(cc.Node)
    nodeNormalBtnAdSpin: cc.Node = null;

    // 黄金按钮
    @property(cc.Node)
    nodeGoldBtnGray: cc.Node = null;
    @property(cc.Node)
    nodeGoldBtnSpin: cc.Node = null;
    ...
    start() {
        ...
        EventCenter.on(EventName.GuideGoldenLuckWheelClick, this.onGoldenSpin, this);
        ...
        this.updateNormalButtonState();
        // 是否需要展示上次转盘的UI
        if (this.getNormalButtonState() == SpinBtnState.Gray) {
            this.showLastLuckWheelUI();
        } else {
            this.showLuckWheelUI();
        }
    ...
    // 普通按钮是否可以点击
    private getNormalButtonState(): SpinBtnState {
        let currentProgress = LuckWheelMgr.Instance.getProgress();
        let targetProgress = LuckWheelMgr.Instance.getTargetCount();
        if (currentProgress >= targetProgress) {
            let cfg = LuckWheelMgr.Instance.getCurrentLuckWheelConfig();
            if (cfg.IsRewardedADShow == 1) {
                return SpinBtnState.AdSpin;
            }
            return SpinBtnState.Spin;
        }
        return SpinBtnState.Gray;
    }

    // 更新普通按钮状态
    private updateNormalButtonState() {
        let buttonState = this.getNormalButtonState();
        this.nodeNormalBtnGray.active = false;
        this.nodeNormalBtnSpin.active = false;
        this.nodeNormalBtnAdSpin.active = false;
        if (buttonState == SpinBtnState.Gray) {
            this.nodeNormalBtnGray.active = true;
        }
        if (buttonState == SpinBtnState.Spin) {
            this.nodeNormalBtnSpin.active = true;
        }
        if (buttonState == SpinBtnState.AdSpin) {
            this.nodeNormalBtnAdSpin.active = true;
        }
    }

    // 显示转盘UI
    private showLuckWheelUI() {
    ...
    private showLastLuckWheelUI() {
        let rewardIndex = LuckWheelMgr.Instance.getLastLuckWheelRewardIndex();
        ...
        let rewardList = LuckWheelMgr.Instance.getLastLuckWheelShowRewardList();
        this.normalLuckWheel.updateUI(rewardList);
    ...
    private updateGoldenLuckWheel(isAnim: boolean = true) {
        LuckWheelMgr.Instance.resetGoldenWheelData();

        // 刷新黄金转盘按钮
        this.nodeGoldBtnGray.active = false;
        this.nodeGoldBtnSpin.active = true;
        ...
        if (isAnim) {
            ...
            this.scheduleOnce(() => {
                ...
                let rewardInfos = LuckWheelMgr.Instance.getCurrentGoldenLuckWheelShowRewardInfo().rewards;
    ...
    // 尝试黄金转盘引导
    private tryGoldenGuide() {
        if (StorageManager.Instance.getBoolean("isGuideGoldenLuckWheel", false)) {
            return;
        }
        StorageManager.Instance.set("isGuideGoldenLuckWheel", true);

        UIManager.Instance.registerTargetNode(TargetNodeKeys.GOLDEN_LUCK_WHEEL_SPIN, this.btnGoldenSpin.node);
        GuideMgr.Instance.changeStep(GuideType.NewUser, 1003);
    }
    ...
    private onNormalSpin() {
        ...
        } else if (buttonState == SpinBtnState.AdSpin) {
            AdMgr.Instance.showAd(ModuleType.LuckWheel, AdType.LuckWheel);
        } else if (buttonState == SpinBtnState.Spin) {
    ...
    private onGoldenSpin() {
        ...
        this.nodeGoldBtnGray.active = true;
        this.nodeGoldBtnSpin.active = false;
        LuckWheelMgr.Instance.addGoldenWheelRotationCount();
    ...
    private onSpin(type: LuckWheelType) {
        ...
        if (type === LuckWheelType.Money) {
            ...
            rewardIndex = LuckWheelMgr.Instance.getCurrentGoldenLuckWheelShowRewardInfo().rewardIndex;
            this.btnGoldenSpin.interactable = false;
            this.btnCloseGolden.interactable = false;
        }
        ...
        cc.tween(nodeWheel)
            .to(3, { angle: totalRotation }, { easing: 'quadInOut' })
            .call(() => {
        ...
        if (reward.value > 0) {
            ...
            EffectManager.instance.playCollectEffect(effectType, 10, startPos, targetPos, () => {
                if (reward.type === RewardType.Money) {
                    ...
                    UserDataMgr.Instance.addTipNumber(reward.value, false);
                }
                // 是否要显示黄金转盘
                if (type === LuckWheelType.Money) {
    ...
    private onCloseGolden() {
        ...
        // if (LuckWheelMgr.Instance.isNeedShowInterstitialAd()) {
        //     LuckWheelMgr.Instance.resetGoldenWheelCloseCount();
        //     AdMgr.Instance.showInterstitialAd(ModuleType.LuckWheel, AdType.LuckWheelClaim);
        // }
        EventCenter.dispatchEvent(EventName.UpdateLuckWheelProgress);
        UIManager.Instance.close(PrefabDefine.PopLuckWheel);
    }
    ...
    onDestroy() {
        ...
        EventCenter.off(EventName.GuideGoldenLuckWheelClick, this.onGoldenSpin, this);
