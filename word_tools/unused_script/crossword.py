...
class Crossword(object):
	...
	def randomize_word_list(self): # also resets words and sorts by length
		...
		self.random.shuffle(temp_list) # randomize word list
	...
	def compute_crossword(self, time_permitted = 1.00, spins=2):
		...
		for r in range(self.rows):
			...
			for col, c in enumerate(self.grid[r]):
				if c != self.empty:
					...
					if col>=maxCol:
						maxCol = col
			if rowContainLetter and r>=maxRow:
				maxRow = r
		...
		for i in range(self.maxRow):
			gridStr += ''.join(self.grid[i][0:self.maxCol])

		hlm = hashlib.md5()
		hlm.update(gridStr.encode(encoding='utf-8'))
		self.hashCode = hlm.hexdigest()
	...
	def sort_coordlist(self, coordlist, word): # give each coordinate a score, then sort
		...
		self.random.shuffle(new_coordlist) # randomize coord list; why not?
	...
	def fit_and_add(self, word): # doesn't really check fit except for the first word; otherwise just adds if score is good
		...
		while not fit and count < self.maxloops:
			...
			if len(self.current_word_list) == 0: # this is the first word: the seed
				...
				vertical, col, row = self.random.randrange(0, 2), 1, 1
				''' 
	...
	def word_find(self): # return solution grid
		...
		for r in range(self.rows):
			for c in self.grid[r]:
				if c == self.empty:
					outStr += '%s ' % string.lowercase[self.random.randint(0,len(string.lowercase)-1)]
				else:
					outStr += '%s ' % c
			outStr += '\n'
		return outStr
...
class WordCrossProblem(object):
	def __init__(self, problemIndex):
		self.result = {};
		self.problemIndex = problemIndex;
	
	def solve(self):
		result = self.result
		word_list = WordCrossProblemData[self.problemIndex-1][0];
		for i in range(2):
			a = Crossword(12, 10, '.', 9999000, word_list)
			a.compute_crossword(20)
			print("compute puzzle {} the {}th time".format(self.problemIndex, i+1))
			print(self.description())
			if abs(a.maxRow - a.maxCol) <=3 and len(a.current_word_list) == len(word_list) and not result.has_key(a.hashCode):
				result[a.hashCode] = a
	def description(self):
		...
		def sortKey(key):
			...
			return oneResult.maxCol * oneResult.maxRow

		keys.sort(key = sortKey)

		resultStr = ""
		for key in keys:
			word = self.result[key]
			resultStr += "========================================solution: {}========================================\n".format(key)
			resultStr += word.solution()
			resultStr += "row: {} col:{}\n".format(word.maxRow, word.maxCol)

		word_list = WordCrossProblemData[self.problemIndex-1][0];
		alphabet_set = set('')
		for word in word_list:
			alphabet_set = alphabet_set|set(word)
		sorted(alphabet_set)
		alphabets = list(alphabet_set)
		alphabetStr = "Alphabets: "+', '.join(str(e) for e in alphabets) + "\n"
		resultStr += alphabetStr

		extraWords = WordCrossProblemData[self.problemIndex-1][1];
		extraWordStr = "ExtraWords: " + ', '.join(str(e) for e in extraWords) + "\n"
		resultStr += extraWordStr

		return resultStr

				
	def log(self):
		filename = "problem_" + str(self.problemIndex).zfill(4) + ".txt"
...
def solve_puzzle(index):
	print('solve_puzzle'+str(index+1))
	problem = WordCrossProblem(index+1)
	problem.solve()
	#problem.log()
...
for index in range(startIndex-1, toIndex):
	t = threading.Thread(target = solve_puzzle, args=(index, ))
	t.setDaemon(True)
	t.start()
	threads.append(t)
