...
export default class RewardMgr {
    ...
    public getLevelFinishRewardConfig() {
        ...
        let rewardString = rewardCfg.LevelFinishReward;
        let rewardList = rewardString.split(';').map(Number);
        if (!rewardList) {
            return 0;
        }
    ...
    // 获取H5奖励
    public getH5RewardConfig() {
        let rewardCfg = this.getRewardConfig();
        if (!rewardCfg) {
            return 0;
        }
        let rewardList = rewardCfg.H5Reward;
        return this.getRandomReward(rewardList);
    }
    ...
    public getFlyBoxRewardConfig() {
        ...
        let rewardString = rewardCfg.WordChestReward;
        let rewardList = rewardString.split(';').map(Number);
        if (!rewardList) {
            return 0;
        }
    ...
    public getRewardConfig(): RewardConfig {
        ...
        if (userTag == UserTag.User101) {
            configKey = ConfigKey.Reward101;
        }
